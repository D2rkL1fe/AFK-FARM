...
func refresh_pet_info(pet: Entity) -> void:
	if is_instance_valid(pet):
		...
		if is_instance_valid(experience_label):
			var current_exp_display = int(pet.current_exp / EXP_DISPLAY_SCALE)
			var exp_to_next_level_display = int(pet.exp_to_next_level / EXP_DISPLAY_SCALE)
		...
		if is_instance_valid(wandering_label):
			var current_wander_exp_display = int(pet.current_wandering_exp / EXP_DISPLAY_SCALE)
			var wander_exp_to_next_level_display = int(pet.wandering_exp_to_next_level / EXP_DISPLAY_SCALE)
			
			wandering_label.text = "WANDER: Lvl " + str(pet.wandering_level) + " (" + str(current_wander_exp_display) + " / " + str(wander_exp_to_next_level_display) + ")"
